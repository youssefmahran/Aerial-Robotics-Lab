...
// Function to make the drone fly in a helix
void flyInHelix(const float x, const float y, const float z, ros::Rate& rate) {
    ...
    for(int i = 1; i < 101; i++){  // Loop to create waypoints for a circular path
        
        des_x = x + 1 * cos(rad(i * 3.6));  // Calculate desired X position
        
        des_y = y + 1 * sin(rad(i * 3.6));  // Calculate desired Y position
        
        des_z = z + i * (1.8 / 360);  // Calculate desired Z position
    ...
    //Go back to center
    des_x = 0;  // Calculate desired X position
        
    des_y = 0;  // Calculate desired Y position
    
    des_z = 1;  // Calculate desired Z position
...
int main(int argc, char **argv) {
    ...
    flyInHelix(curr_x, curr_y, curr_z, rate);  // Fly in a helical path
